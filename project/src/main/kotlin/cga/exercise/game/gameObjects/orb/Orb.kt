...
class Orb:Transformable(), GameObject {
    ...
    override fun update(dt: Float, beat: Float) {
        val speed = basespeed * (beat%1 - 0.5f).pow(4) + 0.2f
        rotateAroundPoint(
    ...
    override fun switchPhase(phase: Phase) {
        when(phase){
            Phase.Day -> {
                basespeed = 8f
                model.renderable.pulseStrength = Random.nextFloat(0.02f)
            }
            Phase.Night -> {
                basespeed = 2f
                model.renderable.pulseStrength = Random.nextFloat(0.1f)
            }
            Phase.Chaos -> {
                model.renderable.pulseStrength = Random.nextFloat(0.1f,0.2f)
                basespeed = 64f
            }
        }
    }
